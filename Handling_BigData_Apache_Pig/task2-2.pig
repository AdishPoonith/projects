...
-- Joining competitors to their medal id
...
-- Grouping by region name to count the number of Gold medals
grouped_gold = GROUP gold_with_regionname BY noc_region::region_name;
gold_medal_count = FOREACH grouped_gold GENERATE group AS region, COUNT(gold_with_region_name) AS gold_count;

-- Grouping by region name to count the number of Silver medals
grouped_silver = GROUP silver_with_regionname BY noc_region::region_name;
silver_medal_count = FOREACH grouped_silver GENERATE group AS region, COUNT(silver_with_region_name) AS silver_count;

-- Left join gold and silver counts together by region
gold_silver_count = JOIN gold_medal_count BY region LEFT OUTER, silver_medal_count BY region;

-- Registering the Python UDF
REGISTER 'task2udf.py' USING jython AS udf;

-- Applying the UDF to fill in missing silver counts
final_output = FOREACH gold_silver_count GENERATE
    FLATTEN(udf.fill_missing_silver(gold_medal_count::region, gold_medal_count::gold_count, silver_medal_count::silver_count)) AS (region, gold, silver);

-- Sorting the final output by gold count , then silver count ,  region name
sorted_output = ORDER final_output BY gold DESC, silver DESC, region ASC;

-- Storing the result in the HDFS output directory
STORE sorted_output INTO '/output/task2-2' USING PigStorage('\t');
